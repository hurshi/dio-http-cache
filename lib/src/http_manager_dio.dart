...
class HttpCacheManager {
  ...
  HttpCacheManager(CacheConfig config) {
  ...
  _onResponse(Response response) async {
    if ((response.request.extra[DIO_CACHE_KEY_TRY_CACHE] ?? false) == false) {
      return response;
    }
    ...
    if (response.statusCode == 304) {
      var responseDataFromCache = await _pullFromCacheBeforeMaxAge(response.request);
      return _buildResponse(responseDataFromCache, response.statusCode, response.request);
    }
    return response;
  }
